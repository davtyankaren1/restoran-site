...
const MenuSection = () => {
  ...
  const additionalDesserts = [
    ...
    },
    {
      ...
      description: "Rich traditional coffee served with Turkish delight",
      price: "$6.95",
      image: coffeeImg
    }
  ];
  ...
  const MenuCategory = ({
    ...
    <div className='mb-8'>
      <h3 className='text-3xl font-light text-foreground mb-12 text-center font-montserrat relative'>
        <span className='relative z-10 bg-background px-6 py-2 rounded-full border border-orangered/20'>
          {title}
        </span>
      ...
      <div className='space-y-6 max-w-4xl mx-auto'>
        {items.map((item) => (
          <div key={item.id} className='relative group'>
            ...
            {/* Content */}
            <div className='relative flex items-center justify-between p-8 rounded-2xl group-hover:scale-[1.02] transition-all duration-300'>
              <div className='flex items-center gap-6'>
                {/* Circular Image with Glow */}
                <div className='flex-shrink-0 relative'>
                  <div className='w-24 h-24 rounded-full overflow-hidden ring-2 ring-orangered/30 group-hover:ring-orangered/50 transition-all duration-300 relative z-10'>
                    <img
                      src={item.image}
                      alt={item.name}
                ...
                <div className='flex-grow'>
                  <h4 className='text-2xl font-medium text-foreground font-montserrat group-hover:text-orangered transition-colors duration-300 mb-3'>
                    {item.name}
                  </h4>
              ...
              <div className='flex-shrink-0 ml-8'>
                <div className='bg-orangered/20 backdrop-blur-sm rounded-full px-6 py-3 border border-orangered/30 group-hover:bg-orangered/30 group-hover:border-orangered/50 transition-all duration-300'>
                  <span className='text-2xl font-semibold text-orangered font-montserrat'>
                    {item.price}
                  </span>
                </div>
              </div>
      ...
      <div className='container mx-auto px-4 relative z-10 max-w-6xl'>
        ...
        <div className='text-center mt-16'>
          <Button
            ...
            className='btn-primary text-lg px-8 py-4 flex items-center gap-2 mx-auto shadow-xl hover:shadow-2xl hover:scale-105 transition-all duration-300'
          >
            {showFullMenu ? (
              <>
                Show Less
