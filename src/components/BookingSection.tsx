...
const BookingSection = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedDate || !formData.name || !formData.email || !formData.time || !formData.guests) {
      toast({
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="relative overflow-hidden rounded-3xl">
          <div className="absolute inset-0">
            ...
            />
            {/* Blur and Orangered Overlay */}
            <div className="absolute inset-0 bg-black/40"></div>
            <div className="absolute inset-0 bg-orangered/30"></div>
                  ...
                  <div className="w-full max-w-none">
                    <Calendar
                      ...
                      classNames={{
                        ...
                        caption: "flex justify-center pt-1 relative items-center text-foreground mb-4",
                        caption_label: "text-xl font-medium",
                        nav: "space-y-1 flex items-center",
                        nav_button: "h-8 w-8 bg-transparent p-0 opacity-50 hover:opacity-100 text-foreground hover:bg-orangered/10 rounded-md transition-colors",
                        ...
                        day_selected: "bg-orangered text-white hover:bg-orangered hover:text-white focus:bg-orangered focus:text-white",
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <Label htmlFor="name" className="text-white/90 font-light">
                      Full Name *
                    </Label>
                    <Input
                      id="name"
                ...
                {/* Phone */}
                <div className="space-y-2">
                  <Label htmlFor="phone" className="text-white/90 font-light">
                    Phone Number
                  </Label>
                  <Input
                    id="phone"
                  ...
                  <div className="space-y-2">
                    ...
                    <Select value={formData.time} onValueChange={(value) => handleInputChange('time', value)}>
                      ...
                      <SelectContent className="bg-white/95 backdrop-blur-sm border-white/30">
                        {timeSlots.map((time) => (
                          <SelectItem key={time} value={time} className="hover:bg-orangered/10">
                            {time}
                          </SelectItem>
                        ))}
                      </SelectContent>
                    ...
                    <Select value={formData.guests} onValueChange={(value) => handleInputChange('guests', value)}>
                      ...
                      <SelectContent className="bg-white/95 backdrop-blur-sm border-white/30">
                        {guestOptions.map((option) => (
                          <SelectItem key={option} value={option} className="hover:bg-orangered/10">
                            {option}
                          </SelectItem>
                        ))}
                      </SelectContent>
                ...
                {/* Message */}
                <div className="space-y-2">
                  <Label htmlFor="message" className="text-white/90 font-light">
                    Special Requests
                  </Label>
                  <Textarea
                    id="message"
